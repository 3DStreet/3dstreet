...
- "Plan view" is removed as a separate view.  This is just a particular camera angle of "Birds eye view".  It is surfaced as a **top-level compass-style button** (analogous to Google Maps' compass), separate from the existing View menu / camera toolbar entries. Clicking it animates the perspective camera to a N-S oriented top-down view, via an **animated transition** (consistent with the smooth transitions used elsewhere; an instant cut would feel jarring). The existing menu and toolbar entries continue to fire the same intercept, so any of them resets to top-down.
