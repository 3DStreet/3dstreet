...
| `TH-21` | `FALLBACK_FORWARD_DIST` | 30 m | Fixed fallback distance when both mesh raycast and ground-plane intersection miss (open sky): `worldPointAt` Step 3 returns this far along the **cursor ray** (not centre-forward — a centre-forward fallback broke sky-grab panning, #1867), and the wheel's level-forward synthetic anchor uses it as its depth. | no | — | 10–50 (feel). |
...
| `TH-78` | `COMPASS_UNBOUNDED_PLAN_VIEW_HEIGHT_METRES` | 200 m | Fallback Plan-View lift height when the scene has no bounds to frame: Plan View rises to at least this altitude (never below the current camera height — Plan View only ever zooms out). Used by `compassController` `handlePlanViewRequest`. | no | — | 100–500 (scene-scale). |
| `TH-79` | `TAIL_DEBOUNCE_MS` | 100 ms | UI tail-debounce on the React nav-mode toolbar: the mode state lags the underlying event by this long so a rapid toggle across `T` (`TH-03`) doesn't visibly flicker the toolbar. The mode-change *event* itself is uncoalesced. **Lives in `useNavMode.js`, not `constants.js`** (a React-side timing knob, like `TH-75`). | no | — | 60–200 (feel). Too short ⇒ boundary flicker; too long ⇒ the toolbar lags a deliberate mode switch. |
| `TH-80` | `WHEEL_ZOOM_OUT_MIN_ANCHOR_DIST_METRES` | 16 m | Zoom-OUT escape floor (#1865): minimum EFFECTIVE camera→anchor distance used to size a zoom-out dolly step. The `TH-08` step is multiplicative in the anchor distance, so a camera parked (near-)on its anchor — reachable via focus on an empty-bbox entity — zoomed out ~millimetres per tick and the wheel read as dead. When zooming OUT closer than this, `cappedDollyStep` sizes the step as if the anchor were this far away (16 × `TH-08` = 0.8 m/tick, matching legacy `minSpeedFactor` × `zoomSpeed`). Zoom-IN is untouched (stays asymptotic); exact in/out reversibility inside this radius is deliberately traded for never-stuck. | no | — | 8–32. Too small ⇒ near-anchor zoom-out still crawls; too large ⇒ the first out-tick from a legitimately-close inspection lurches. |
