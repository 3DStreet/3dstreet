...
// - removeMember frees each slot back to the BatchedMesh via deleteInstance (not just hidden),
//   so a later addInstance reuses the id. Duplicates added AFTER the batch pass (editor clone /
//   layer reorder recreate entities) come through onLateModelLoaded unbatched; trackLateUnbatched
//   tallies them per key and, past LATE_BATCH_THRESHOLD, repackLateUnbatched folds them into the
//   matching group — or builds a fresh one — reusing freed slots and growing capacity with
//   setInstanceCount, so a long editor session doesn't accrete unbatched draw calls.
...
function addMemberToBatchedMeshes(group, el) {
  ...
  for (const { batchedMesh, entries } of group.batchedMeshes) {
    ensureInstanceCapacity(batchedMesh, entries.length);
...
// the BatchedMesh. deleteInstance frees each slot back to the BatchedMesh's internal
// _availableInstanceIds pool (and stops it rendering), so a later addInstance — e.g. a
// late-added duplicate folded in by repackLateUnbatched — reuses that id instead of growing
// the buffer. So repeated remove/add cycles reclaim slots rather than leaking holes. Also
// clears the raycast mapping so a removed/reparented entity no longer raycasts to a
// detached object3D.
