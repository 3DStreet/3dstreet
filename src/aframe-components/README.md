...
| `segments-changed` | `managed-street`                                               | `{ changeType: 'structure', added, removed }` or `{ changeType: 'property', property: 'width' \| 'length' \| 'showBuildings', segment, oldValue, newValue }` (`segment` is `null` for length and showBuildings changes, since the change is at the managed-street level) | yes     |
