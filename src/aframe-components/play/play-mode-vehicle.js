...
AFRAME.registerSystem('play-mode-physics', {
  ...
  activate: async function () {
    ...
    // Expose the loaded module so other play features (e.g.
    // play-mode-helicopter) can build their own body/collider descs
    // without a second import of the WASM chunk.
    this.RAPIER = RAPIER;
    this.active = true;
  },
...
AFRAME.registerComponent('drive-mode', {
  ...
  onPlayStart: function () {
    ...
    physics.activate().then(() => {
      ...
      this._vehicleColliderListeners = seedObstacleColliders(sceneEl, [
        driveEntity,
        car
      ]);
    });
