...
describe('batch-models slot reclamation (tier 2)', () => {
  it('removeMember frees the slot back to the pool and keeps the group alive', () => {
    ...
    expect(bm._availableInstanceIds).toContain(id1); // slot returned to the pool
    expect(bm.instanceCount).toBe(1);
...
describe('batch-models capacity (tier 3 primitives)', () => {
  ...
  it('addMemberToBatchedMeshes reuses a freed hole before growing capacity', () => {
    const { bm, geometryId, geometry } = makeBatchedMesh(2);
    const sceneEl = fakeSceneEl();
    const group = makeGroup(bm, geometryId, geometry, sceneEl);
    ...
    // Free m1's slot, then add a fresh member: it must reuse the hole, not grow.
    batch.removeMember(m1);
    expect(bm._availableInstanceIds.length).toBe(1);

    const m3 = fakeEl();
    batch._test.addMemberToBatchedMeshes(group, m3);
    expect(bm.maxInstanceCount).toBe(2); // no growth — hole reused
    expect(bm._availableInstanceIds.length).toBe(0);
...
describe('batch-models late-batch tally (tier 3)', () => {
  ...
  it('accumulates duplicates but does not fold below the threshold', () => {
    ...
    const group = (() => {
      ...
      const g = makeGroup(bm, geometryId, geometry, sceneEl);
      sceneEl._batchModelsBuilt.push(g);
      return g;
    })();
