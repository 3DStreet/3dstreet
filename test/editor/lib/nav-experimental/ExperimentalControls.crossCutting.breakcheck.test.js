// Break-it checks (KD-4b acceptance bar). Each re-runs a WE9 / WE11 two-arm
// proxy's core assertion with the specific invariant hand-disabled and asserts
// the proxy now RED-alarms. A proxy that stays green with its invariant
// disabled is worthless; these files are the proof it doesn't.
//
// Disablement targets (PLAN §2 table):
//   WE9 zoom-undo cleared / WE11 cross-controller invalidation → _clearZoomUndo no-op
//   WE9 grounded re-derived → _deriveGroundedFromPose no-op
...
// Run the WE9 zoom-undo two-arm and return |tiltA − tiltB| (the discriminating
...
describe('break-it — WE9 zoom-undo proxy reds when _clearZoomUndo is disabled', () => {
  ...
  it('the proxy FAILS with _clearZoomUndo disabled (arms collapse together)', () => {
    ...
    // retrace → the difference vanishes → the WE9 assertion would red.
...
// WE9 grounded two-arm: the discriminating quantity is (yFlying − yGrounded)
// after driving down a slope. `disableDerive` makes _deriveGroundedFromPose a
// no-op for the grounded arm, so grounded is never set true and both arms hold.
...
describe('break-it — WE9 grounded proxy reds when _deriveGroundedFromPose is disabled', () => {
...
// WE11 cross-controller invalidation: same disablement target (_clearZoomUndo),
// but the intervening move is a rotation.
...
describe('break-it — WE11 cross-controller invalidation reds when _clearZoomUndo is disabled', () => {
