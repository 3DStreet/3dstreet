...
**Geospatial:** `street-geo`, `google-maps-aerial`, `tiled-basemap` (tiled 2D raster basemap on 3d-tiles-renderer; providers/keys via `src/tested/basemap-providers.js` + `config/.env.*`, MapTiler primary — #1962; legacy `mapbox2d` migrates to it at load), `osm-buildings` (2.5D extruded OSM buildings; worker-driven Overpass pipeline in `src/osm/` — fetch/cache modules there are shared with future OSM import (#1930); worker code must never import `three`), `geojson`, `geo-flatten`
