...
- `fly-mode` + `play-mode-helicopter` — GTA-style arcade helicopter (attitude-command flight model in `heli-flight-model.js`, pure + unit-tested); spawns from a `[fly-controls]` entity (procedural `helicopter-mesh` visual); shares the Rapier world and the street/obstacle collider seeding (`scene-colliders.js`) with drive-mode; if both a drive and a fly entity exist, drive wins the session
