...
- `managed-intersection` - **Prototype**: Y-up (rotation `0 0 0`), auto-generates surface/curb-wedges/crosswalks/signals from the managed-street endpoint nodes within `snapRadius`; rebuilds live via an interval signature watch (editor pauses ticks). Geometry math is pure/unit-tested in `src/tested/managed-intersection-utils.js`; entry point `docs/managed-intersection.md`
