...
**Geospatial:** `street-geo`, `google-maps-aerial`, `tiled-basemap` (tiled 2D raster basemap on 3d-tiles-renderer; providers/keys via `src/tested/basemap-providers.js` + `config/.env.*`, MapTiler primary — #1962; legacy `mapbox2d` migrates to it at load; in osm3d mode also drapes the OpenMapTiles `transportation` layer as a second MVTOverlay-textured surface — styling in `src/tested/osm-street-style.js`), `osm-buildings` (2.5D extruded OSM buildings from the basemap provider's vector tiles — MapTiler `tiles/v3` building layer, decoded in `src/tested/vector-tile-buildings.js`; worker pipeline in `src/osm/`, load radius centered on the camera's look-at ground point; the Overpass fetch/cache modules there are no longer used by buildings but are kept for the phase-6 hydrator (#1930); worker code must never import `three`), `osm-streets` (#1930 demo path: streams decoded transportation way records near the camera and powers **click-to-upgrade** — an empty-space editor click near a way offers "Upgrade to 3DStreet street" via `OsmUpgradeChip`, minting real `playable` managed streets from class presets in `src/tested/osm-street-import.js`; roadmap: `docs/osm-street-lod.md`), `geojson`, `geo-flatten`
