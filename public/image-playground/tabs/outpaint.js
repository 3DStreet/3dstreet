...
const OutpaintTab = {
        ...
        this.elements.outpaintContainer.innerHTML = `
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 bg-white rounded-lg shadow p-6">
                    ...
                    <div class="mb-4 param-group">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Safety Tolerance: <span id="outpaint-safety-value">2</span></label>
                        <input type="range" id="outpaint-safety-slider" min="0" max="6" step="1" value="2" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer">
                    ...
                    <!-- Advanced Options -->
                     <div class="mb-4">
                        <div class="flex justify-between items-center cursor-pointer" id="outpaint-advanced-toggle">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" id="outpaint-advanced-icon">
    ...
    getElements: function() {
        this.elements.fileInput = this.elements.outpaintContainer.querySelector('#outpaint-file-input');
        this.elements.fileNameLabel = this.elements.outpaintContainer.querySelector('#outpaint-file-name');
        this.elements.previewContainer = this.elements.outpaintContainer.querySelector('#outpaint-preview-container');
        this.elements.previewImage = this.elements.outpaintContainer.querySelector('#outpaint-preview-image');
        this.elements.sourceImage = this.elements.outpaintContainer.querySelector('#outpaint-source-image');
        this.elements.originalSizeLabel = this.elements.outpaintContainer.querySelector('#outpaint-original-size');
        this.elements.newSizeLabel = this.elements.outpaintContainer.querySelector('#outpaint-new-size');

        // Expansion inputs
        this.elements.topInput = this.elements.outpaintContainer.querySelector('#outpaint-top');
        this.elements.bottomInput = this.elements.outpaintContainer.querySelector('#outpaint-bottom');
        this.elements.leftInput = this.elements.outpaintContainer.querySelector('#outpaint-left');
        this.elements.rightInput = this.elements.outpaintContainer.querySelector('#outpaint-right');
        this.elements.expansionInputs = [this.elements.topInput, this.elements.bottomInput, this.elements.leftInput, this.elements.rightInput];

        // Parameters
        this.elements.promptInput = this.elements.outpaintContainer.querySelector('#outpaint-prompt');
        this.elements.stepsSlider = this.elements.outpaintContainer.querySelector('#outpaint-steps-slider');
        this.elements.stepsValue = this.elements.outpaintContainer.querySelector('#outpaint-steps-value');
        this.elements.guidanceSlider = this.elements.outpaintContainer.querySelector('#outpaint-guidance-slider');
        this.elements.guidanceValue = this.elements.outpaintContainer.querySelector('#outpaint-guidance-value');
        this.elements.safetySlider = this.elements.outpaintContainer.querySelector('#outpaint-safety-slider');
        ...
        this.elements.advancedOptions = this.elements.outpaintContainer.querySelector('#outpaint-advanced-options');
        this.elements.advancedIcon = this.elements.outpaintContainer.querySelector('#outpaint-advanced-icon');
        this.elements.promptUpsamplingInput = this.elements.outpaintContainer.querySelector('#outpaint-prompt-upsampling');
        this.elements.formatJpeg = this.elements.outpaintContainer.querySelector('#outpaint-format-jpeg');
        this.elements.formatPng = this.elements.outpaintContainer.querySelector('#outpaint-format-png');

        // Output
        this.elements.outputContainer = this.elements.outpaintContainer.querySelector('#outpaint-output-container');
        this.elements.outputPlaceholder = this.elements.outpaintContainer.querySelector('#outpaint-output-placeholder');
        this.elements.outputImage = this.elements.outpaintContainer.querySelector('#outpaint-output-image');
        this.elements.loadingIndicator = this.elements.outpaintContainer.querySelector('#outpaint-loading-indicator');
        this.elements.loadingText = this.elements.outpaintContainer.querySelector('#outpaint-loading-text');

        // Buttons
        this.elements.generateBtn = this.elements.outpaintContainer.querySelector('#outpaint-generate-btn');
        this.elements.actionButtons = this.elements.outpaintContainer.querySelector('#outpaint-action-buttons');
        // this.elements.addToGalleryBtn = this.elements.outpaintContainer.querySelector('#outpaint-add-to-gallery-btn'); // Removed
        this.elements.copyParamsBtn = this.elements.outpaintContainer.querySelector('#outpaint-copy-params-btn');
        this.elements.downloadBtn = this.elements.outpaintContainer.querySelector('#outpaint-download-btn');
    },
    ...
    setupEventListeners: function() {
        ...
        this.elements.generateBtn.addEventListener('click', this.generateOutpaint.bind(this));
    ...
    setInputImage: function(imageDataUrl) {
        ...
        this.elements.sourceImage.onload = () => { // Ensure image is loaded before using dimensions
    ...
    resetOutput: function() {
        ...
        this.elements.outputPlaceholder.textContent = this.imageData ? 'Set expansion dimensions' : 'Load an image and set expansion';
    ...
    handleSuccess: function(imageUrl, resultData) {
        ...
        this.currentParams.model = resultData.details?.model_id || 'flux-pro-1.0-expand'; // Add model info
    ...
    generateOutpaint: async function() {
        ...
        if (top === 0 && bottom === 0 && left === 0 && right === 0) {
            FluxUI.showNotification('Please enter pixels to add in at least one direction.', 'warning');
            return;
        }
        if (top < 0 || bottom < 0 || left < 0 || right < 0) {
             FluxUI.showNotification('Expansion values cannot be negative.', 'warning');
            return;
        }
         if (top > 2048 || bottom > 2048 || left > 2048 || right > 2048) {
             FluxUI.showNotification('Expansion values cannot exceed 2048.', 'warning');
            return;
        }
...
// Initialize the Outpaint tab when the DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    window.OutpaintTab = OutpaintTab; // Make globally available
    OutpaintTab.init();
});